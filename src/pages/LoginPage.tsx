...
import { Zap, Mail, Phone, Lock, Eye, EyeOff, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    const identifier = loginMethod === "email" ? email : mobile + "@mobile.user";
    if (!identifier || !password) {
      setError("Please fill in all fields");
      return;
    }
    login(identifier, password, roleFromState);
    navigate(roleFromState === "retailer" ? "/retailer" : "/home");
  };

  const handleGoogleLogin = () => {
    login("google.user@gmail.com", "", roleFromState);
    navigate(roleFromState === "retailer" ? "/retailer" : "/home");
  };
        ...
        <div className="bg-card/90 backdrop-blur-xl rounded-2xl border border-border/50 shadow-2xl p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Sign in to your {roleFromState === "retailer" ? "Retailer" : "Customer"} account
            </p>
          </div>
          ...
          <div className="flex bg-muted rounded-xl p-1 mb-6">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${
                loginMethod === "email" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${
                loginMethod === "mobile" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
            >
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ...
            <Link to="/signup" state={{ role: roleFromState }} className="text-primary font-semibold hover:underline">
              Create Account
            </Link>
          </p>
        </div>
